...
__global__ void transpose_batch_backward(uint32_t *d_transposed, uint32_t *d_poly, int N_size, int BATCH) {
    __shared__ uint32_t tile[32][33];
    int x = blockIdx.x * 32 + threadIdx.x; 
    int y = blockIdx.y * 32 + threadIdx.y; 
    if (x < BATCH && y < N_size) {
        tile[threadIdx.y][threadIdx.x] = d_transposed[y * BATCH + x];
    }
    __syncthreads();
    
    int trans_x = blockIdx.y * 32 + threadIdx.x; 
    int trans_y = blockIdx.x * 32 + threadIdx.y; 
    if (trans_x < N_size && trans_y < BATCH) {
        d_poly[trans_y * N_size + trans_x] = tile[threadIdx.x][threadIdx.y];
    }
}
